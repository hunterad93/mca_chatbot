...
def format_citation(annotation):
    ...
    if filename:
        # Extract the title, chapter, and part from the filename
        parts = filename.split('---')
        if len(parts) == 3:
            title = parts[0].replace('title_', '')
            chapter = parts[1].replace('chapter_', '')
            part = parts[2].replace('part_', '').replace('.md', '')
            # Construct the URL using the extracted values
            file_url = f"https://leg.mt.gov/bills/mca/title_{title}/chapter_{chapter}/part_{part}/sections_index.html"
